...
def StudentAccountApi():
    file = open("userfile.txt", "a")
    with open("userfile.txt", "r") as fp:
        x = len(fp.readlines())
    if(x<=9):
        return
    else:
        for line in open("studentAccounts.txt", "r").readlines():
            if(line == "===="):
                continue
            else:
                userapi = line.split()
                file.write(userapi[0])
                file.write(" ")
                file.write(userapi[1])
                file.write(" ")
                file.write(userapi[2])
                file.write(" ")
...
def MyCollege_users():
    file = open("MyCollege_users", "a")
    for line in open("userfile.txt", "r").readlines():
        userapi = line.split()
        file.write(userapi[0])
        file.write(" ")
        file.write(userapi[6])
        file.write("====")
        file.write("\n")
